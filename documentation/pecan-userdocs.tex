...
\subsubsection{Predicate Expressions}
In a addition to the aforementioned expressions, Pecan provides users with a construct called \emph{predicate expressions}, which are defined in the following way:
\begin{verbatim}
    min { <var name> : <predicate> }
    max { <var name> : <predicate> }
    inf { <var name> : <predicate> }
    sup { <var name> : <predicate> }
\end{verbatim}
They can be used as variables in arithmetic expressions and will represent the respective minimum / maximum / infimum / supremum of the set of elements satisfying the predicate.

\subsubsection{Distinctness}
If you need to ensure variables are distinct, you can achieve it by using the following syntax:
\begin{verbatim}
    distinct { <variables> }
\end{verbatim}
Note that the variables must be given as a comma-separated list.
...
For each variable in the variable list, you can also write \verb+<variable> is <type>+ to inform Pecan of the types of the variable.
This may be useful if you do not use global type restrictions.
